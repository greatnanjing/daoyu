...
- **折算四处口径一致**（出站熔断按页计数的延伸）：`outbox_sent_pages` / `db.sent_pages_today` 加 `md_clean_enabled` 参数，文本行折算同过 md_clean——运行时计数 / 重启恢复 / 日界重算 / bridge `/status` 折算四处统一传 `bool(cfg.throttle.get("md_clean", True))`。
- **开关**：`throttle.md_clean`（bool 默认 true，`_DEFAULT_THROTTLE` + config.example.json 同构）——`/config set` **首个 bool 键**（值认 true/false、parser 转布尔、JSON 写回布尔；既有数值键解析路径不动），白名单 16 键；关闭则原文直发，重启生效（清洗误伤一键回退）。
