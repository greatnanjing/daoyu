...
- **日期**: 2026-08-21
- **状态**: 已实现并真机验收通过（2026-08-21，434 测试全绿；连发 2 段合并单 prompt/排队位次 ACK/窗口机制全过）
...
验收通过（435 测试全绿 + 生产服务器 + 微信真机）：连发合并全链路（首条 ACK「正在合并后续消息」→ 窗口内追加静默 → 到点「已合并 N 条，开始处理」拼单 prompt）、`/config set throttle.merge_window_s` 可调生效、B 队列位次 ACK（「当前任务完成后接上，你排在第 M 位」）真机验证通过；语音转写文字同走合并路径（生产语义）。崩溃恢复（KV 暂存重启 flush、「已恢复」措辞）由单测 + E2E 钉死，未做真机专项演练（恢复路径与常规 flush 共享代码）。
