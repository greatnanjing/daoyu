...
- **日期**: 2026-08-21
- **状态**: 已实现并真机验收通过（2026-08-21，419 测试全绿；PDF 收发/语音转写/send_file 视频条可播放/文件条原名/入站视频存盘+降级提示全过）
...
验收通过（419 测试全绿 + 生产服务器 + 微信真机）。§5 验收点六项全过（发文件 / 语音转写 / 视频入站 / send_file mp4 / send_file xlsx / 大文件）：出站 video/file 条真机形态成立（mp4 视频条微信端可播放、文件条保留原名可下载）；入站文件/语音/视频三分支按设计落盘建任务、语音转写即文字；100MB 上传走 outbox 重试链路无异常；无缩略图视频微信端渲染正常（官方同形态预期兑现）。

**验收期发现**：服务器无 ffmpeg（base 源没有，EPEL/rpmfusion 未启）——入站视频的抽帧由 Claude 用 Chrome video 元素截图自行完成，工具兜底成立；装 ffmpeg 保持为可选运维项、非必需。
