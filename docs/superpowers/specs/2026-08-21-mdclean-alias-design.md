...
## 7. 真机验收结论与设计修订（2026-08-21）

**五项全过**：①清洗观感 ②`t(u)` 可点性 ③`/alias add go`→`/go` 全链路 ④`/t` `/cs` 秒回 ⑤开关双态（false+重启原文直发实证）。嵌套引用 spec 偏差已修（`cade11b`）、15 项 Minor 全清（`10c6de2`）、Linux 侧平台性测试债清偿（`ab8d687`，双平台 487+486 全绿）。

**验收期重大发现——「微信不渲染 Markdown」立项前提被推翻**：微信新版**手机+PC 双端一致原生渲染** Markdown（`##`→黑体标题、`**`→粗体、表格正常可读、代码块吞围栏保留内容）。开关双态对照实证：清洗开 = 转写吃掉可渲染语法（用户看到纯文本无粗体）；清洗关 = 原文直发享受原生渲染（用户看到粗体/标题/表格）。

**设计修订**：`throttle.md_clean` 默认值 true→**false**（fallback 四处 + 折算签名默认同步翻转，`outbox_sent_pages`/`sent_pages_today` 签名默认 False、`_DEFAULT_THROTTLE` + config.example.json 同构）——**清洗降级为逃生通道**（老客户端/某类消息渲染异常时 `/config set throttle.md_clean true` 一键开回）。md_clean 纯函数与全部清洗机制保留不删，行为测试以显式开关驱动。
